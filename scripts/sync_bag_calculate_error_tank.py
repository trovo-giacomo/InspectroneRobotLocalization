...
def callback(cam1, cam2, cam3, ekf, tagSlam):
    ...
    ekf_pub.publish(ekf)
    tagSlam_pub.publish(tagSlam)

    ## CAM 1
    # calculate MSE of the position wrt the ground truth of the Optitrack
    reference_pose = np.array([tagSlam.pose.pose.position.x, tagSlam.pose.pose.position.y, tagSlam.pose.pose.position.z])
...
if __name__ == '__main__':
    ...
    tagSlam_pub = rospy.Publisher("/sync/tagslam/odom/body_rig", Odometry, queue_size=10)
    ...
    tagSlam_sub = message_filters.Subscriber("/tagslam/odom/body_rig", Odometry)

    ts = message_filters.ApproximateTimeSynchronizer([cam1_sub, cam2_sub, cam3_sub, ekf_sub, tagSlam_sub], 10, 0.1)
